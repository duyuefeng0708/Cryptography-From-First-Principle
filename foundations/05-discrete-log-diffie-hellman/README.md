...
- Small-subgroup attack on DH with an unsafe prime: force the shared secret into a small subgroup
- Pohlig-Hellman on a smooth-order group: recover the secret exponent by solving small DLPs
- Recover a shared secret from leaked partial bits using lattice-based or meet-in-the-middle techniques

## Connect

Real-world appearances in `connect/`:
- Diffie-Hellman in TLS 1.3 key exchange: the ephemeral DH handshake that establishes session keys
- DH in the Signal protocol (X3DH): how the extended triple Diffie-Hellman protocol bootstraps end-to-end encryption
